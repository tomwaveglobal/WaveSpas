...
class DropdownElement extends HTMLElement {
  constructor() {
    ...
    this.detectHoverLeavePanelListener = this.detectHoverLeavePanel.bind(this);
    ...
    this.addEventListener('mouseleave', this.detectHoverLeavePanelListener);
  ...
  detectHoverLeavePanel(event) {
    if (theme.config.isTouch) return;
    if (!this.open) return;
    const next = event.relatedTarget;
    if (next && (this.contains(next) || this.controls.some((c) => c.contains(next)))) {
      return;
    }
    this.hide();
  }
...
class SecondaryMedia extends HTMLElement {
  ...
  init() {
    ...
    if (this.mediaCount > 1) {
      ...
      // The card is wrapped in an <a> to the product page, so dot taps would
      // navigate away. Block propagation on dot clicks so only Flickity sees them.
      this.addEventListener('click', this.onClickHandler);
    }
  }

  onClickHandler(event) {
    if (event.target.closest('.flickity-page-dot')) {
      event.preventDefault();
...
class ProductInfo extends HTMLElement {
  ...
  updatePreOrderMessagesVisibility(variantId) {
    const messages = this.querySelectorAll('.pre-order-message-block[data-pre-order]');
    if (!messages.length) return;

    const idStr = variantId != null ? String(variantId) : '';
    messages.forEach((el) => {
      const match = el.getAttribute('data-pre-order') === idStr;
  ...
  applyVariantFiltering(selectedOptionValues = []) {
    ...
    // DOM uses Shopify option.position (1-based); variant.options[] is 0-based
    for (let i = 0; i < optionCount; i++) {
      const position = i + 1;
      const selectedInput = root.querySelector(`[data-option-index="${position}"]:checked`);
    ...
    for (let i = 0; i < optionCount; i++) {
      ...
      const optionInputs = root.querySelectorAll(`[data-option-index="${i + 1}"]`);
    ...
    // Apply swatch disabled states after filtering
  ...
  handleUpdateProductInfo() {
    return (parsedHTML) => {
      ...
      if (!variant) {
        this.setUnavailable();
        this.updatePreOrderMessagesVisibility(null);
        return;
      }
      ...
      this.updatePreOrderMessagesVisibility(variant.id);
...
class ComparisonTable extends HTMLElement {
  constructor() {
    ...
    this.wrapper = this.closest('.comparison-table-wrapper');
    this.controlsScope = this.wrapper?.parentElement || this.parentElement;
    this.scrollPrevButton = this.controlsScope?.querySelector('[data-comparison-scroll-prev]');
    this.scrollNextButton = this.controlsScope?.querySelector('[data-comparison-scroll-next]');
    this.onScrollCueHandler = theme.utils.throttle(this.updateScrollCue.bind(this));
    this.onScrollPrevHandler = this.scrollPrev.bind(this);
    this.onScrollNextHandler = this.scrollNext.bind(this);
  ...
  connectedCallback() {
    this.addEventListener('scroll', this.onScrollCueHandler);
    window.addEventListener('resize', this.onScrollCueHandler);
    this.scrollPrevButton?.addEventListener('click', this.onScrollPrevHandler);
    this.scrollNextButton?.addEventListener('click', this.onScrollNextHandler);
    this.updateScrollCue();
  }

  disconnectedCallback() {
    this.removeEventListener('scroll', this.onScrollCueHandler);
    window.removeEventListener('resize', this.onScrollCueHandler);
    this.scrollPrevButton?.removeEventListener('click', this.onScrollPrevHandler);
    this.scrollNextButton?.removeEventListener('click', this.onScrollNextHandler);
  }
  ...
  scrollByStep(direction = 1) {
    ...
    const scrollStep = Math.max(Math.round(this.clientWidth * 0.75), 240);
    const nextScrollLeft = Math.min(Math.max(this.scrollLeft + scrollStep * direction, 0), maxScrollLeft);
  ...
  updateScrollCue() {
    ...
    const isScrolledStart = this.scrollLeft <= 8;
    const isScrolledEnd = this.scrollLeft >= maxScrollLeft - 8;

    this.classList.toggle('has-overflow', hasOverflow);
    this.classList.toggle('is-scrolled-start', isScrolledStart);
    this.classList.toggle('is-scrolled-end', isScrolledEnd);

    if (this.wrapper) {
      this.wrapper.classList.toggle('has-overflow', hasOverflow);
      this.wrapper.classList.toggle('is-scrolled-start', isScrolledStart);
      this.wrapper.classList.toggle('is-scrolled-end', isScrolledEnd);
    }

    if (this.scrollPrevButton) {
      this.scrollPrevButton.toggleAttribute('disabled', !hasOverflow || isScrolledStart);
    }

    if (this.scrollNextButton) {
      this.scrollNextButton.toggleAttribute('disabled', !hasOverflow || isScrolledEnd);
    }
  }
